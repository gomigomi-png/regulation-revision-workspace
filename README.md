...
このリポジトリでは、採用管理サンプルを土台に **規程改正ワークスペース** を作っています。
`npm run dev` で開く `http://localhost:3000` に出るのは、採用管理ではなく規程改正の画面です。

規程改正のドキュメントは `docs/` にあります。この README はひな形の手引なので、規程改正の作業は次を見てください。

| ドキュメント | 内容 |
|---|---|
| [作業ガイド](docs/regulation-revision-workspace-guide.md) | ファイルのありか・開き方・GitHub 作業 |
| [検討メモ](docs/regulation-revision-workspace-notes.md) | 設計判断（目的・方針・ペイン責務） |
| [図解](docs/regulation-revision-visual-explainer.md) | 画面・データ・出力の流れ |
...
ブラウザで `http://localhost:3000` を開きます。ひな形のままなら採用管理サンプルが出ます。このリポジトリでは規程改正ワークスペースが出ます。
